<?php
	...
	if ($node->status==0) {
		echo "<p class='unpublished'>This competition is UNPUBLISHED and will not be visible to users</p>";
	} 
?>
...
<div<?php print $id_node . $classes; ?>>
	...
	<?php print $content ?>
  	<?php 
 		...
 		if (!$loggedin) {?>
  		<p class="info">To enter this competition you need to <a href="/user/register/">join our community</a> or <a href="/user/login?destination=node/<?php print $node->nid;?>">login</a> if you have already registered.</p>
  	<?php }?>
</div>
<?php } ?>